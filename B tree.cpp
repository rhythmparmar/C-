...
int main()
{
    btree  b;// ob create then a constructor call
    int ch, key, flag;
    system("clear");
    while(1)
    {
...
{
    int i,j, t, max;
    NODE temp = find_leaf(root,key);// first step to find where key is suppose to go?
    if(!temp)
    {
        cout<<"Error!: Cannot insert: This integer is already present in the tree\n";
        return;
    }
    if(!isFull(temp)) // if(!0)// yes there is a place vacant at temp
    {
    ...
    if(isFull(temp)) // no place in this
    {
        split(temp);
        insert(key);
    }
